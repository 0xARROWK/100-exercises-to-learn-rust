...
pub async fn pong(mut receiver: mpsc::Receiver<Message>) {
    loop {
        // With normal channel, runtime will be blocked until recv() receive a message.
        // Moreover, as there is not yield point (.await) in the loop, we will never yield control
        // back to the runtime, enter the next loop iteration and block again on the recv() method.
        // This iteration will repeats again and again, leading to a runtime blocked forever.
